...
Meeting::Meeting(const Meeting &t_meeting) {
	...
	std::vector<string> v(t_meeting.m_participators);
	m_participators = v;
}
...
void Meeting::setParticipator(const std::vector<std::string> t_participators) {
	std::vector<string> v(t_participators);
	m_participators = v;
}
...
bool Meeting::isParticipator(const std::string t_username) const {
	for ( vector<std::string>::const_iterator it = m_participators.begin() ; it != m_participators.end() ; it++ ) {
		if ( *it == t_username )
			return true;
	}
	return false;
